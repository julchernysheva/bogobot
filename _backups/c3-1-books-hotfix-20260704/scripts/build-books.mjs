...
export function renderMarkdown(markdown, options = {}) {
  ...
  while (index < lines.length) {
    ...
    if (heading) {
      ...
      html.push(`<h${level} id="${escapeAttribute(anchor)}">${renderInline(title, options)}</h${level}>`)
      index += 1
      continue
    }
...
function renderNavigationParts(item, sections, manifest) {
  const sectionLinks = sections.map((section, index) => `<a href="#${escapeAttribute(section.anchor)}" data-section-kind="${escapeAttribute(section.kind)}"><span>${String(index + 1).padStart(2, "0")}</span><strong>${escapeHtml(section.title)}</strong></a>`).join("")
  ...
  return { sectionLinks, concepts }
}

function renderRubricator(item, sections, manifest) {
  const { sectionLinks, concepts } = renderNavigationParts(item, sections, manifest)
  return `<aside class="book-rubricator" aria-label="Рубрикатор и ключевые понятия"><div class="book-rubricator-sticky"><nav class="book-section-nav" aria-label="Разделы книги"><span class="book-section-label">РУБРИКАТОР</span>${sectionLinks}</nav>${concepts ? `<nav class="book-concept-nav" aria-label="Ключевые понятия"><span class="book-section-label">КЛЮЧЕВЫЕ ПОНЯТИЯ</span>${concepts}</nav>` : ""}</div></aside>`
}

function renderMobileNavigation(item, sections, manifest) {
  const { sectionLinks, concepts } = renderNavigationParts(item, sections, manifest)
  return `<div class="book-mobile-navigation" aria-label="Навигация по странице"><details class="book-mobile-nav-group"><summary><span>РУБРИКАТОР</span><small>${String(sections.length).padStart(2, "0")}</small></summary><nav class="book-section-nav" aria-label="Разделы книги">${sectionLinks}</nav></details>${concepts ? `<details class="book-mobile-nav-group"><summary><span>КЛЮЧЕВЫЕ ПОНЯТИЯ</span><small>${String((item.concepts || []).length).padStart(2, "0")}</small></summary><nav class="book-concept-nav" aria-label="Ключевые понятия">${concepts}</nav></details>` : ""}</div>`
}
...
<body>
  ...
  <header class="books-topbar">
    <a class="books-logo" href="${escapeAttribute(booksHref)}" aria-label="Книги Bogobot"><img src="${escapeAttribute(logoHref)}" alt="BOGOBOT"></a>
...
<body>
  ...
  <header class="books-topbar">
    <a class="books-logo" href="../" aria-label="Карта Bogobot"><img src="../assets/logo.gif" alt="BOGOBOT"></a>
...
export function createBooksOutputs(manifest = readBooksManifest()) {
  ...
  manifest.route.forEach((item, index) => {
    ...
    const withoutLead = removeLeadMarkdown(sourceDocument.markdown, item.leadMarkdown)
    outputs.set(item.output, renderReaderPage(manifest, item, index, {
      ...sourceDocument,
      markdown: filterBookOnlySections(withoutLead),
