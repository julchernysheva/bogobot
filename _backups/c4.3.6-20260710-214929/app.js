...
function renderWorldNavigation() {
  ...
  const visible=!activeMapMode&&state.filter==="world"
  nav.hidden=!visible
  $(".app").classList.toggle("world-navigation-open",visible)
  if(!visible) return
  const focusId=activeVisualFocusId(state.current)
  const activeIndex=worldNavigationIds.indexOf(focusId)
  const activeRecord=activeIndex>=0?byId[focusId]:null
  $("#worldNavigationCount").textContent=`${worldNavigationIds.length} GRAPH NODES`
  $("#worldNavigationState").textContent=activeRecord?`ACTIVE: ${activeRecord.title}`:"SELECT A WORLD NODE"
  $("#worldNavigationSummary").textContent=activeRecord
    ?`${worldNavigationIds.length} NODES / ${activeRecord.title}`
    :`${worldNavigationIds.length} NODES`
  const list=$("#worldNavigationItems")
  list.replaceChildren(...worldNavigationIds.map(id=>{
    const record=byId[id]
    const button=document.createElement("button")
    button.type="button"
    button.dataset.worldNodeId=id
    button.className=id===focusId?"active":""
    button.setAttribute("aria-current",id===focusId?"true":"false")
    button.title=record.title
    button.innerHTML=`<span>${id}</span><small>${record.title}</small>`
    return button
  }))
  $("#worldPrevious").disabled=activeIndex<=0
  $("#worldNext").disabled=activeIndex<0||activeIndex>=worldNavigationIds.length-1
  if(innerWidth>900) $("#worldNavigationDisclosure").open=true
}
...
function categoryOverviewIds(currentId=state.current) {
  ...
  return new Set(graphNodes
    .filter(node=>state.discovered.has(node.id)
      &&nodeBelongsToFilter(node,state.filter)
...
function lensFitProfile(mode,readerOpen,categoryIds) {
  ...
  const contract=categoryActive&&!readerOpen
    ?fitContractForCount(categoryIds.size,{readerOpen:false})
    :null
  return {
...
function fitContractForCount(count,{readerOpen=false,mobile=false}={}) {
  ...
  return {occupancy:.62,maxScale:2.2}
}
...
function drawGraph() {
  ...
  renderedGraphEdges().forEach(({node,targetId,waypoints}) => {
    ...
    const edge = waypoints.length
      ?makeSvg("polyline",{
        points:[sourcePosition,...waypoints,targetPosition].map(point=>`${point.x},${point.y}`).join(" "),
        class:edgeClass
      })
      :makeSvg("line",{
...
function fitDesktopMap(mode,id=state.current,correcting=false,retry=false) {
  ...
  if(isValidGraphTransform(cachedTransform)){
    ...
    const primaryIds=categoryIds?[...categoryIds]:[...new Set([id,...display.continuationSet.map(record=>record.id),...display.clusterDisplayIds])]
    const contextIds=categoryIds?[]:[...display.contextIds]
    $("#graph").setAttribute("viewBox",geometry.viewBox)
    viewport.style.transform=cachedTransform
    recordFitDiagnostic(mode,{
      paneWidth:geometry.paneWidth,
  ...
  focusFrame=requestAnimationFrame(()=>{
    ...
    const paddingLeft=Math.max(paddingPx,fitProfile.leftPadPx||0)*viewWidth/paneWidth
    const paddingRight=Math.max(paddingPx,fitProfile.rightPadPx||0)*viewWidth/paneWidth
    const paddingTop=Math.max(Math.min(72,Math.max(56,paneHeight*.08)),fitProfile.topPadPx||0)*viewHeight/paneHeight
    const paddingBottom=Math.max(Math.min(72,Math.max(56,paneHeight*.08)),fitProfile.bottomPadPx||0)*viewHeight/paneHeight
...
globalThis.__bogobotFitDiagnostics=()=>JSON.parse(JSON.stringify(Object.fromEntries(fitDiagnostics)))
