...
try{
  ...
  assert.equal(await button.getAttribute("aria-label"),"Как читать")
  assert.equal((await button.textContent()).trim(),"КАК ЧИТАТЬ")
  ...
  assert.equal(guide.guideH1,"КАК ЧИТАТЬ")
